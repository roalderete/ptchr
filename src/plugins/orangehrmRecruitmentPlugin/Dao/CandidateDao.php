...
class CandidateDao extends BaseDao
{
     ...
     */
    public function findLatestInterviewAttachmentByInterviewId(int $interviewId): ?InterviewAttachment
    {
        $qb = $this->createQueryBuilder(InterviewAttachment::class, 'ia');
        
        $qb->where('ia.interview = :interviewId'); 
        $qb->setParameter('interviewId', $interviewId);
        
        // Order by the attachment ID descending to get the most recently uploaded attachment
        $qb->addOrderBy('ia.id', 'DESC'); 
        $qb->setMaxResults(1);
    ...
    /**
     * Finds the latest Interview entity for a given CandidateVacancy ID.
     * @param int $candidateVacancyId
     * @return Interview|null
     */
    public function findLatestInterviewByCandidateVacancyId(int $candidateVacancyId): ?Interview
    {
        $qb = $this->createQueryBuilder(Interview::class, 'i');
        
        // FIX APPLIED: Using the correct entity property name: 'candidateVacancy'
        $qb->where('i.candidateVacancy = :candidateVacancyId'); 
        
        $qb->setParameter('candidateVacancyId', $candidateVacancyId);
        
        // Order by interview ID descending to get the 'latest' one
    ...
    /**
     * Finds the latest 'note' string from the CandidateHistory (ohrm_job_candidate_history) 
     * for a given Candidate ID and Action ID (like RECRUITMENT_APPLICATION_ACTION_OFFER_JOB).
     * @param int $candidateId
     * @param int $action The action ID (e.g., 7 for OFFER_JOB)
     * @return string|null
     */
    public function findLatestHistoryNoteByCandidateAndAction(int $candidateId, int $action): ?string
    {
        $qb = $this->createQueryBuilder(CandidateHistory::class, 'history');
        
        $qb->select('history.note');
        
        $qb->where('history.candidate = :candidateId');
        $qb->setParameter('candidateId', $candidateId);
        
        $qb->andWhere('history.action = :actionId');
        $qb->setParameter('actionId', $action);
        
        // Ensure we get the latest entry in case multiple offer notes were recorded
        $qb->addOrderBy('history.id', ListSorter::DESCENDING);
        $qb->setMaxResults(1);

        try {
            // getSingleScalarResult() fetches the value of the single selected column (history.note)
            // and returns it as a string or null if no result is found.
            $result = $qb->getQuery()->getSingleScalarResult();
            
            // Check if the result is not false (which getSingleScalarResult might return if null/empty)
            if ($result !== false && $result !== null) {
                return (string) $result;
            }
        } catch (Exception $e) {
            // Handle cases where no record is found or the query fails
            error_log("Failed to find history note for candidate {$candidateId} and action {$action}: " . $e->getMessage());
        }
