...
class CandidateActionController extends AbstractVueController
{
    ...
    public function preRender(Request $request): void
    {
        ...
        if ($actionId === WorkflowStateMachine::RECRUITMENT_APPLICATION_ACTION_OFFER_JOB) {
            
        $interviewIds = $this->getCandidateService()->getCandidateDao()->getInterviewIdsByCandidateId($candidateId);
        $latestInterviewId = !empty($interviewIds) ? $interviewIds[0] : null;

        $component->addProp(
            new Prop('latest-interview-id', Prop::TYPE_NUMBER, $latestInterviewId)
        );
        $component->addProp(
