...
class CandidateHistoryAPI extends Endpoint implements CrudEndpoint
{
    ...
    public function getAll(): EndpointResult
    {
        ...
        $candidate = $candidateService->getCandidateDao()->getCandidateById($this->getCandidateId());


        if ($candidate) {
            /** @var CandidateHistory $record */
            foreach ($candidateHistoryRecords as $record) {
                // Check for the JOB_OFFERED action AND verify the interview link is null
                if (
                    $record->getAction() === WorkflowStateMachine::RECRUITMENT_APPLICATION_ACTION_OFFER_JOB
                    && is_null($record->getInterview())
                ) {
                    // Manually fetch the latest *successful* interview entity
                    // NOTE: The getLatestCandidateInterview method in CandidateService MUST be 'protected' or 'public'.
                    $latestInterview = $candidateService->getLatestCandidateInterview($candidate);
                    
                    if ($latestInterview) {
                        // Temporarily set the Interview entity on the history record.
                        // When CandidateHistoryListModel serializes $record, it will now find the ID.
                        $record->setInterview($latestInterview); 
                    }
                }
            }
        }
