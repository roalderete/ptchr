...
class EmailService
{
    ...
    public function getMessage()
    {
        ...
        if (empty($this->messageFrom)) {
            ...
            $this->messageFrom = [$this->getEmailConfig()->getSentAs() => "PTC-HR"];
        }
        ...
        foreach ($this->messageAttachments as $attachment) {
            // Use the inherited Symfony\Component\Mime\Email::attach() method
            $message->attach($attachment['data'], $attachment['filename'], $attachment['contentType']);
        }
        return $message;
    }
    ...
    public function sendTestEmail(string $toEmail): bool
    {
        ...
        $body = "This email confirms that $type details set in PTC-HR ";
    ...
    /**
     * Stores raw attachment data to be added to the email message later.
     * @param string $data
     * @param string $filename
     * @param string $contentType
     */
    public function addRawAttachment(string $data, string $filename, string $contentType): void
    {
